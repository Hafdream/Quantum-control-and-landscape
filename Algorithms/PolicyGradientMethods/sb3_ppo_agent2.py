...
def train_agent(env, total_time_steps, lr, exploration_fraction, weights_path, verbose):
    ...
    agent = sb3.PPO('MlpPolicy', env, policy_kwargs=dict(net_arch=custom_net_arch), n_epochs=20, gamma=0.99,
...
def run_agent(env, episode_max, weights_path):
    ...
    file_path = "../../results_old/4param_PPO_rand_initialization2_rand2.csv"
    df.to_csv(file_path, index=False)

    df2 = pd.DataFrame(fid_and_pulse_from_PCA_loadings)
    file_path2 = "../../results_old/4param_PPO_rand_initialization_pca_loadings2.csv"
...
if __name__ == "__main__":
    for zz in [4]:  # [2, 3, 4, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50]:
        ...
        weights_path = "./ppo_agent_4param_num_partitions_20000000.zip"
        # train_agent(env, total_time_steps=5000000, lr=0.0001, exploration_fraction=0.15, weights_path=weights_path,  verbose=1)
        train_agent(env, total_time_steps=2000000, lr=0.0001, exploration_fraction=0.15, weights_path=weights_path,
                    verbose=1)
