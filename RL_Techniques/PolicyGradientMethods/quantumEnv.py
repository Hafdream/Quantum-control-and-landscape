...
class QuantumControlEnv(gym.Env):
    def __init__(self, num_actions, num_partitions, max_infidelity, is_train):
    ...
    def step(self, action):
        ...
        if self.action_rand[pulse_amplitude] >= 100*self.num_partitions and self.nstep == 0 and self.is_train:
            ...
            print(f"ACTION_RAND:{pulse_amplitude}, {self.action_rand[pulse_amplitude]}")
        ...
        if err < self.max_infidelity and self.action_list and (self.action_list[0] not in self.tmp_buffer) and (self.nstep == self.num_partitions):
            ...
            rwd = -100
            print(f"MIN RWD: {rwd}")
        # elif err < self.max_infidelity and self.action_list and (self.action_list[0] in self.tmp_buffer):
        #     rwd = 1 * (err < 0.5) + 10 * (err < 0.15) + -10 * (err < self.max_infidelity)
        # elif self.nstep == 1 and (pulse_amplitude in self.tmp_buffer):
        #     rwd = -10 * (err < 0.75) + -10 * (err < 0.15) + -10 * (err < self.max_infidelity)
        #     print(f"Min RWD: {rwd}")
        else:
        ...
        terminate = (err < self.max_infidelity) and (self.nstep >= self.num_partitions)
